import svgPaths from "../../imports/svg-uoumltnu0f";
...
function EmailInput() {
  ...
  const arrowColor = focused ? "#F5F5F5" : email.trim() ? "#AF63FF" : "#9293BA";

  return (
    <div
      className="flex items-center justify-between border border-[#f5f5f5] border-solid pl-[16px] pr-[8px] py-[8px] rounded-[4px] shrink-0 w-[277px] focus-within:border-[#af63ff] transition-colors duration-200"
      data-name="List"
    >
      <input
        type="email"
      ...
      <button
        type="button"
        className="cursor-pointer bg-transparent border-none p-0 transition-all duration-200"
...
export default function Footer({ variant = "purple" }: { variant?: "dark" | "purple" }) {
  const bgColor = variant === "dark" ? "bg-[#110846]" : "bg-[#300067]";

  return (
    <div className={`${bgColor} h-[717px] overflow-clip relative shrink-0 w-[1440px]`} data-name="Footer">
      <ProductsContainer />
